        ...
        AzHostActivityLayout(navController = navController, initiallyExpanded = false) {
            ...
            onscreen(alignment = Alignment.TopEnd) {
                if (!uiState.hideUiForCapture) AzDropdownMenu(navController = navController) {
                    ...
                    // Every entry here is the same colour, and `azConfig` has no hook to say so once —
                    // so it was said twenty times, and one entry that missed it fell back to
                    // Color.Unspecified and rendered unreadable against the menu. Said once now.
