...
class KritaBrushStagesTest {
    ...
    fun `masked tip can have independent pressure dynamics`() {
        ...
        // Primary radius is unaffected by the masked brush's pressure dynamics (only taper/fade and
        // per-dab speed sensitivity apply). Mid-stroke dabs have resting radius scaled by speed factor.
        val midDabs = dabs.filter { it.x > 10f && it.x < 35f }
        midDabs.forEach { assertTrue(it.radius > 8f && it.radius <= 10f) }
    }
}
