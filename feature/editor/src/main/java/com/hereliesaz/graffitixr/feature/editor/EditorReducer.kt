...
import com.hereliesaz.graffitixr.common.model.ColorStyle
...
import com.hereliesaz.graffitixr.common.model.TextStyle
...
internal object EditorReducer {
    ...
    fun reduce(state: EditorUiState, intent: EditorIntent): EditorUiState {
        return reduceLayerProperties(state, intent)
            ?: reduceLayerTransforms(state, intent)
            ?: reduceLayerList(state, intent)
            ?: reduceEditorSettings(state, intent)
    ...
    private fun reduceLayerProperties(state: EditorUiState, intent: EditorIntent): EditorUiState? = when (intent) {
    ...
    private fun reduceLayerTransforms(state: EditorUiState, intent: EditorIntent): EditorUiState? = when (intent) {
    ...
    private fun reduceLayerList(state: EditorUiState, intent: EditorIntent): EditorUiState? = when (intent) {
    ...
    private fun reduceEditorSettings(state: EditorUiState, intent: EditorIntent): EditorUiState? = when (intent) {
        ...
        is EditorIntent.SetBrushSizeFixedOnScreen -> state.copy(brushSizeFixedOnScreen = intent.value)
    ...
    private fun reduceBrushSettings(state: EditorUiState, intent: EditorIntent): EditorUiState? = when (intent) {
    ...
    private fun reduceComponents(state: EditorUiState, intent: EditorIntent): EditorUiState? = when (intent) {
    ...
    private fun reduceLayoutAndStyles(state: EditorUiState, intent: EditorIntent): EditorUiState? = when (intent) {
    ...
    private fun EditorUiState.restyled(
        colorStyles: List<ColorStyle> = this.colorStyles,
        textStyles: List<TextStyle> = this.textStyles,
    ): EditorUiState = copy(
