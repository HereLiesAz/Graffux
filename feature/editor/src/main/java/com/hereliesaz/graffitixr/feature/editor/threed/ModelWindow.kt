 ...
 * separate mode.
 *
 * Phase 1 is inspection only: load a model and orbit it. Painting onto its surface is the next
 * phase, and the UV data needed for it is already parsed and uploaded.
 */
@Composable
fun ModelWindow(
    state: ModelUiState,
    ...
    FloatingWindow(title = state.name ?: "3D Model", onDismiss = onDismiss) {
        when {
            ...
            state.mesh != null -> {
                ModelView(mesh = state.mesh, modifier = Modifier.fillMaxWidth().height(260.dp))
                Text(
