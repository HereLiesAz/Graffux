...
**Live preview and Vulkan parity: investigated this pass, deliberately still not attempted, and now for a concrete, checked reason rather than a general "no design decision was made" placeholder.** Item 13's airbrush got its live-preview gap closed this same pass by tracking held dabs as a second incremental append-only queue — cheap, since it's still strictly proportional to the *new* dabs each frame. Impasto doesn't have an equivalent cheap increment available: `ImpastoEngine.shade()` (called once per commit today) takes the **entire** `colorPixels`/`heightMap` arrays, not a region — `DrawingEngine`'s commit path already calls it exactly once per stroke, over the whole layer, relying on the algorithm's own "a flat/unpainted pixel is provably unchanged" guarantee to keep that affordable at commit time. Calling it on every live-preview drag frame (tens of times per stroke, not once) would turn an O(width×height) pass that's fine once into a very different cost profile during an active drag — a real interactivity risk on anything larger than a small test canvas, not a hypothetical one. Making it a live-preview citizen properly needs either a regional/incremental variant of `shade()` (an actual algorithm change, not a wiring change) or a throttled/dirty-rect re-shade strategy (a real behavior tradeoff — visibly stepped shading during a fast drag vs. full smoothness) — a design decision, the same category item 16's tile-based undo storage is deliberately left at, not a "played it safe" placeholder. GPU/Vulkan parity is downstream of the same problem: there is no live-preview CPU version of Impasto to port to a shader yet (unlike item 3's dilution, which already had one), and GPU live preview is a dab-compositing accelerator only (item 15) — adding height-map accumulation and relief shading to it would mean building this feature's live-preview story from scratch on the GPU, not porting an existing verified CPU one.
...
**Vulkan target:** Not started — no equivalent exists in `stamp.comp` or `VulkanColorSmudge`, and (checked this pass, see above) there's no live-preview CPU version to port from yet either, unlike item 3's `dilution`.
