...
**Design for the actual next step (proposed, not yet implemented): a tile-delta fast path for the common single-step undo/redo case, layered on top of the existing replay model rather than replacing it.** The invariant established above — a stroke still on `EditHistory`'s undo stack is guaranteed to still be in `LayerStore`'s un-baked stroke list, always as that layer's *most recent* stroke at the moment it's undone — is exactly what makes this safe: `EditCommand.Draw` could gain an optional `tileDeltas: List<TileDelta.TileSnapshot>?` (null = no fast path, unconditional fallback to today's full `rebuildLayerBitmap()` replay, so this is purely additive and never a regression), captured once, when a stamp-brush stroke's authoritative post-commit bitmap is computed (the same async recompute `commitStampStroke` already performs — not the live-preview bitmap, which is presentation-only and shouldn't become an undo-correctness source), using that stroke's own `DirtyRegion.fromDabs()`-derived `TileGrid.tilesTouching()` range. A single-step undo of a stroke with captured deltas becomes `TileDelta.applyBefore()` on the *current* live bitmap directly — O(that stroke's touched tiles), not O(every stroke since base) — with `onUndoClicked()`/`onRedoClicked()` falling back to the existing full-replay path whenever `tileDeltas` is null (non-stamp tools, or any stroke type this pass doesn't cover) or the fast path's own preconditions aren't met. **Not attempted this pass**: retrofitting `commitStampStroke`'s "push history synchronously, recompute asynchronously" control flow to attach a tile-delta captured only after that async recompute finishes is a real change to a concurrency pattern several other things in `EditorViewModel` depend on (seed guards against a fast-restarted stroke, `clearTransientStrokeState` races) — verifying every interaction correctly needs more room than this pass had, and getting undo/redo subtly wrong is a correctness bug users would experience as their own painting silently corrupting, not an abstract risk worth rushing past. Backward compatibility with anything already on the undo stack is explicitly NOT a constraint the user asked to preserve here, unlike other items in this document — there's no serialized undo history to begin with (nothing survives an app restart today), so this is a purely in-session design question, not a migration one.

**Determinism/replay:** N/A yet — none of `DirtyRegion`/`TileGrid`/`TileDelta` touches replay; nothing consumes their output. Once something does, tile boundaries and dirty-region bookkeeping must not change replayed pixel output, per this item's original framing (performance/memory optimization only, not a semantic change). `TileDelta` itself is a pure function of its inputs (same before/after/tile-range in, same snapshots out; same snapshots + target in, same restored target out) — verified by a lossless round-trip test (`applyBefore` then `applyAfter` on the same snapshots reproduces the original "after" state exactly, for a non-uniform pixel pattern and a tile size that doesn't evenly divide the canvas).

**CPU reference:** `DirtyRegion.fromDabs()`/`.union()`/`.clampTo()`, `TileGrid.tileBounds()`/`.tilesTouching()`, and `TileDelta.capture()`/`.applyBefore()`/`.applyAfter()` — all implemented and tested.
...
**Tests:** `DirtyRegionTest.kt` — single-dab bounds, multi-dab union, secondary masked-tip extent inclusion, `union()`, `clampTo()` (including producing an empty region when entirely outside canvas), and `width`/`height`/`isEmpty`. `TileGridTest.kt` — column/row rounding for a canvas not evenly divisible by tile size, edge-tile clamping, out-of-range `tileBounds()`, single- and multi-tile `tilesTouching()`, the exclusive-edge case, clamping a region straddling the canvas boundary before computing indices, and the empty-range case for a region entirely outside the canvas. `TileDeltaTest.kt` (new) — an empty touched range or a mismatched-size buffer both capture nothing (defensive, not a crash); `capture()` pulls exactly the touched tiles' pixels in row-major order; `applyBefore()` restores only the touched tiles and leaves the rest of the canvas untouched; `applyAfter()` is `applyBefore()`'s inverse; a round trip (`capture` → `applyBefore` → `applyAfter`) is lossless for a non-uniform pixel pattern on a tile size that doesn't evenly divide the canvas.

**Dependencies:** Items 3, 15 (touches the same layer read/modify/write surface).

**Completion state:** IN PROGRESS. Dirty-region bounding-box, tile-index mapping, and tile-delta capture/replay primitives: IMPLEMENTED (CPU), tested, no consumer yet. A tile-delta fast path for single-step undo/redo (the actual next step, now concretely designed above and grounded in real Krita/Procreate research): NOT STARTED — the remaining blocker is verifying the concurrency retrofit to `commitStampStroke`'s history-push/async-recompute split correctly, not a missing design.
