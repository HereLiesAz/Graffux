...
`gpuCompatibleStampBrush()` in `EditorViewModel` no longer excludes grain at all — only a masked/dual-brush config still forces CPU. `gpuPipelineUsesMaskedShader()` now also routes a round tip *with* grain to `stamp_masked.comp` (previously only a shaped tip or non-round `tipRatio` triggered it), since `stamp.comp` has no grain sampler binding whatsoever — `stamp_masked.comp` already handles a null/round tip correctly via `BrushTipMaskCache.tipMask(null, ...)`'s generated round mask, the same path a shaped tip's absence already exercised.

**Determinism/replay:** GPU path must reproduce CPU output for the same resolved dab record and the same resolved grain tile/phase — unverified at runtime for both the shaped-tip and grain additions: they now have live callers, but nothing in this session's environment can compare actual GPU output against the CPU reference (see Vulkan target below). `resolveGrainTileAndPhase()`'s determinism itself (same seed → same phase) is verified by test.
...
**Vulkan target:** `stamp_masked.comp` + `VulkanStampEngine::stampMaskedDabs()`, now wired into the live stamp-brush stroke pipeline for shaped tips, non-round `tipRatio`, and grain — compiles clean via the NDK's `glslc` and full arm64-v8a/armeabi-v7a native builds, but **has never run on an actual GPU**: nobody working on this repo in this session has a physical Vulkan-capable device (the same hardware gap item 17 tracks), so correctness beyond "the shader compiles, the C++ builds, and the Kotlin call site type-checks" is unverified. A wrong result on real hardware would surface as a visibly wrong stroke, not a crash — the same fallback contract as the round-only path means a hard GPU failure (init/dispatch error) still degrades gracefully to CPU, but a shader logic bug that runs "successfully" with wrong pixels would not be caught by anything in this repo today. The `MOVING` grain behavior's exact rotation semantics (see above) are a particular point of risk for this reason — it was reasoned through by reading the CPU code, not confirmed by rendering both and comparing. Masked/dual-brush sampling remains entirely unstarted.

**UI exposure:** None new in this step — existing shape/tip-ratio/grain controls (already exposed via `BrushStudioWindow`, items 5-6) simply stop forcing CPU for the live preview once a Vulkan-capable device is present; there's no new UI surface, only a live-preview performance path that was previously unavailable for these brushes.

**Tests:** `GpuCompatibleStampBrushTest.kt` covers both gate functions, updated for this pass: a shaped tip, a non-round `tipRatio`, and now a round tip with grain are all GPU-compatible via the masked pipeline; only a masked/dual-brush config still forces CPU. `AlphaChannelBytesTest.kt` (prior pass) covers the mask-texture byte-packing helper, reused as-is for grain's byte packing. `ResolveGrainTileAndPhaseTest.kt` (new) covers the shared phase-resolution function: no grain source resolves to a null tile and zero phase, a fixed offset with no random component resolves directly, `grainRandomOffsetPerStroke` adds a deterministic extra offset reproducible from the same seed, and different seeds draw different offsets. None of this exercises the actual GPU dispatch or shader correctness — that needs CPU/GPU parity tests analogous to the existing Color Smudge parity/benchmark instrumentation (item 3), run on real hardware, which remains undone.

**Dependencies:** Items 3, 5-7.

**Completion state:** IN PROGRESS. Shaped-tip, non-round-`tipRatio`, and texture/grain sampling: engine primitives written, compile-verified, and wired into the live stroke pipeline (`gpuCompatibleStampBrush()`/`gpuPipelineUsesMaskedShader()` route eligible strokes to it, with the same fail-safe CPU fallback contract as the round-only path) — but NOT runtime-verified on any GPU, since nobody working on this repo has physical Vulkan-capable hardware. Masked/dual-brush sampling: NOT STARTED.
