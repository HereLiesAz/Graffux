...
This is still explicitly the smaller, safer half of the item, deliberately so. Tile-based *undo storage* does not exist: `EditHistory.kt`/`LayerStore.kt` remain entirely whole-bitmap-snapshot based, unchanged by this — `TileDelta` has no consumer in either. Wiring an actual undo-storage strategy on top of it (deciding, per edit, when a tile-delta entry is safe/beneficial vs. falling back to a whole-bitmap snapshot; threading that decision through every `pushDraw`/undo/redo call site; migrating or coexisting with already-saved whole-bitmap history) is a materially larger, correctness-sensitive change to how every stroke's undo/redo works — real user data at stake if it's wrong — and is explicitly NOT attempted here. Unlike the physical-hardware gaps this document is explicit about NOT treating as an excuse (items 15/17), this is a genuinely different kind of open question: it needs its own design pass, not more device access, and `TileDelta`'s correctness as a pure, tested capture/replay primitive doesn't by itself settle that design question (encoding/compression, memory-vs-CPU tradeoffs at different tile sizes, and how it should interact with `LayerStore`'s existing base+strokes replay model are all still open). `DirtyRegion`, `TileGrid`, and now `TileDelta` are the foundation a future tile-based undo (or dirty-region-aware partial redraw/partial GPU upload) could build on, not that thing itself.
...
**Completion state:** IN PROGRESS. Dirty-region bounding-box, tile-index mapping, and tile-delta capture/replay primitives: IMPLEMENTED (CPU), tested, no consumer yet. Tile-based undo *storage* (the part that actually replaces `EditHistory`/`LayerStore`'s whole-bitmap snapshots): NOT STARTED — needs its own design pass (when to use a tile delta vs. a whole snapshot, encoding, migration/coexistence with existing history), not blocked on anything this session could unblock by "not playing it safe."
